...
void switchOnOff( FILE *f, char *path, struct stat *statbuf, int value)
{
	...
	fprintf(f, "<HTML><HEAD><TITLE>Switch state</TITLE></HEAD>\r\n<BODY>");
	fprintf(f, "<HR>\r\n");
	fprintf(f, "<A HREF=\"..\">Switch state %d</A>\r\n", value);
	fprintf(f, "<HR>\r\n</BODY></HTML>\r\n");
}
...
void process(FILE *f)
{
	...
	if (strcasecmp(method, "GET") != 0) {
		...
		if (strncmp(path,"/sw_on",6) == 0) {
			printf("switch on\n");
			switchOnOff( f, path, &statbuf, 1);
		}
		else if (strncmp(path,"/sw_off", 7) == 0)
		{
			printf("switch off\n");
			switchOnOff( f, path, &statbuf, 0);
		}
		else
